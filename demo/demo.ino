...
void yello_team() {
  ...
  if (millis() - pidtest_time < 2000 and flag == 3 and lai == 0) {
    PIDL2();
//    pidtest_time = millis();
  } else if (flag == 3) {
    if (millis() - pidtest_time < 2500) {
      Motor_reset();
      lai = 1;
    } else {
  ...
  if (distance_R < 45 and flag == 4 and lai == 0 and digitalRead(plus_ball_over_pin) == LOW) {
    PIDB();
    pidtest_time = millis();
  } else if (flag == 4) {
